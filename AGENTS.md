...
- There is no lint script or configured e2e target. The README's `ng e2e` text is Angular CLI boilerplate, not an available workflow.
...
- Firebase project `recipe-app-a7be0` is the live backend: Google Auth, Firestore, Storage, and the `importRecipeWithAi` Cloud Function. The browser client reads the web config from `src/environments/environment.ts`; there is no frontend `.env` loading or Angular file replacement. `apiKey` and `appId` must be filled from the registered web app before sign-in works.
...
- AI image import calls `environment.firebase.importRecipeUrl`, implemented by `functions/src/index.ts`. The function checks the Firebase ID token and rejects users without the pro claim. `OPENAI_API_KEY` is a Functions secret. The old Deno function in `supabase/functions/import-recipe-with-ai/index.ts` is no longer called by the app.
- `scripts/migrate-supabase-to-firebase.mjs` copies Supabase users, recipes, and images into Firebase by email. Run `npm run migrate:supabase-to-firebase -- --dry-run` first. `scripts/migrate-firebase-to-supabase.mjs` is the historical reverse script. The service-role key and Firebase service-account JSON are server-only secrets.
